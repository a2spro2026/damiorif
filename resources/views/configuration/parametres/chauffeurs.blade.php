...
<style>
    ...
    .modal-sheet { width:min(560px,100%); background:linear-gradient(160deg,rgba(84,0,11,.97),rgba(45,0,6,.98)); border:1px solid rgba(94,200,179,.35); border-radius:18px; padding:1.5rem; }
...
<div class="content-panel fiche-page">
    ...
    <div class="table-wrap">
        <table class="data-table">
            <thead><tr><th>Nom</th><th>Téléphone</th><th>CIN</th><th>Action</th></tr></thead>
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                <button type="button" class="icon-btn" onclick='openEdit(@json($c))'><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></button>
                                <form method="POST" action="{{ route('configuration.parametres.chauffeurs.destroy', $c) }}" onsubmit="return confirm('Supprimer ?');" style="display:inline;">@csrf @method('DELETE')
                ...
                @empty
                    <tr class="empty-row"><td colspan="4">Aucun chauffeur.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-backdrop" id="modal"><div class="modal-sheet">
    ...
    <form method="POST" id="form">@csrf<input type="hidden" name="_method" id="formMethod" value="POST">
        <div class="form-grid">
            <div class="field full"><label>Nom</label><input name="nom" id="field_nom" required></div>
            <div class="field"><label>Téléphone</label><input name="telephone" id="field_tel"></div>
            <div class="field"><label>CIN</label><input name="cin" id="field_cin"></div>
        </div>
...
const storeUrl=@json(route('configuration.parametres.chauffeurs.store'));
const updateBase=@json(url('/configuration/parametres/chauffeurs'));
function openModal(){modal.classList.add('open')} function closeModal(){modal.classList.remove('open')}
function openCreate(){document.getElementById('modalTitle').textContent='Ajouter un chauffeur';form.action=storeUrl;document.getElementById('formMethod').value='POST';document.getElementById('field_nom').value='';document.getElementById('field_tel').value='';document.getElementById('field_cin').value='';openModal()}
function openEdit(c){document.getElementById('modalTitle').textContent='Modifier';form.action=updateBase+'/'+c.id;document.getElementById('formMethod').value='PUT';document.getElementById('field_nom').value=c.nom||'';document.getElementById('field_tel').value=c.telephone||'';document.getElementById('field_cin').value=c.cin||'';openModal()}
