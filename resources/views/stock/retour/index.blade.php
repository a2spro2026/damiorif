...
<style>
    ...
    .form-row-meta { grid-template-columns:160px 160px minmax(0,1fr); }
...
<div class="content-panel fiche-page">
    ...
    <form method="POST" action="{{ route('stock.retour.store') }}" id="retourForm" class="form-panel">
        ...
        <div class="form-row form-row-remarque">
            <div class="field">
                ...
                <textarea name="remarque" id="field_remarque" required placeholder="Justifier le motif du retour…">{{ old('remarque') }}</textarea>
            </div>
        </div>
    ...
    <h3 class="section-title">Historique des retours</h3>
    <div class="table-wrap">
        <table class="data-table">
            ...
            <tbody>
                @forelse ($retours as $mvt)
                    @php
                        $qte = (float) $mvt->lignes->sum('quantite');
                        $src = $depotLabels[$mvt->depot] ?? $mvt->depot;
                        $remarque = $mvt->note ?? '';
                ...
                @empty
                    <tr class="empty-row"><td colspan="6">Aucun retour enregistré.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
