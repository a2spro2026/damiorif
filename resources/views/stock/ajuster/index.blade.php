...
<style>
    ...
    .form-row-meta { grid-template-columns:160px 160px minmax(180px,1fr); }
    ...
    .hint { color:var(--text-muted); font-size:.82rem; margin:.35rem 0 .75rem; }
    ...
    .icon-btn { width:32px; height:32px; border-radius:8px; border:1px solid rgba(94,200,179,.3); background:var(--bg-input); color:var(--gold); display:inline-flex; align-items:center; justify-content:center; cursor:pointer; }
    ...
    @media (max-width:800px) { .form-row-meta { grid-template-columns:1fr 1fr; } }
    @media (max-width:560px) { .form-row-meta { grid-template-columns:1fr; } }
</style>
...
<div class="content-panel fiche-page">
    <div class="page-toolbar">
        ...
        <div class="toolbar-actions">
            <a href="{{ route('stock.depot') }}" class="btn btn-ghost">Stock Dépôt</a>
    ...
    <form method="POST" action="{{ route('stock.ajuster.store') }}" id="ajusterForm" class="form-panel">
        @csrf
        ...
        <div class="form-row form-row-meta">
            ...
            <div class="field">
                ...
                <select name="depot" id="field_depot" required>
                    @foreach ($depots as $key => $label)
                        <option value="{{ $key }}" @selected(old('depot', $preselectDepot) === $key)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <p class="hint">Qté positive = augmente le stock · Qté négative = diminue le stock (ex. −5).</p>
        ...
        <div class="totals-bar">
            <span>Qté nette : <strong id="totalQte">0</strong></span>
            <span>Montant : <strong id="totalMontant">0,00</strong></span>
        </div>

        <div class="form-footer">
            <a href="{{ route('stock.depot') }}" class="btn btn-ghost">Fermer</a>
            <button type="submit" class="btn btn-gold">Valider</button>
        </div>
    </form>

    <h3 class="section-title">Derniers ajustements</h3>
    <div class="table-wrap">
        <table class="data-table">
            ...
            <tbody>
                @forelse ($ajustements as $mvt)
                    @php
                        $qte = (float) $mvt->lignes->sum('quantite');
                        $depot = $depotLabels[$mvt->depot] ?? $mvt->depot;
                        $remarque = $mvt->note ?? '';
                ...
                @empty
                    <tr class="empty-row"><td colspan="6">Aucun ajustement enregistré.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<script>
    ...
    function addLine(data = {}) {
        ...
        tr.innerHTML = `
            ...
            <td><input type="number" step="0.01" class="js-qte" name="lignes[${i}][qte]" value="${data.qte ?? 1}" required></td>
        ...
        tr.querySelector('.js-ref').addEventListener('change', () => applyFromRef(tr));
        tr.querySelector('.js-ref').addEventListener('blur', () => applyFromRef(tr));
        tr.querySelector('.js-designation').addEventListener('change', () => applyFromDes(tr));
        tr.querySelector('.js-designation').addEventListener('blur', () => applyFromDes(tr));
    ...
    function recalcLine(tr) {
        ...
        tr.querySelector('.js-st').value = formatMoney(Math.abs(qte) * pu);
        recalcTotals();
    }
    ...
    document.getElementById('ajusterForm').addEventListener('submit', e => {
        ...
        for (const tr of linesBody.querySelectorAll('tr')) {
            const designation = (tr.querySelector('.js-designation')?.value || '').trim();
            const qte = parseFloat(tr.querySelector('.js-qte')?.value);
            if (!designation) continue;
            hasLine = true;
            if (!qte) {
                e.preventDefault();
                alert('La quantité d\'ajustement ne peut pas être 0.');
                return;
            }
        }
        if (!hasLine) {
