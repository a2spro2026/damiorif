...
<div class="content-panel fiche-page">
    <div class="page-toolbar">
        <div>
            <h2>Balance Fournisseurs</h2>
            <div class="page-meta">
                @if ($depotLabel)
                    Dépôt : {{ $depotLabel }} —
                @endif
                {{ $rows->count() }} fournisseur{{ $rows->count() > 1 ? 's' : '' }}
            </div>
        </div>
    ...
    @include('partials.kpi-grid', ['cards' => [
        ['label' => 'Total Cmd', 'value' => $totalCmd, 'id' => 'kpiCmd', 'integer' => true],
        ['label' => 'Total Montants', 'value' => $totalMontant, 'id' => 'kpiMontants', 'unit' => 'MAD'],
    ...
    <div class="table-wrap">
        <table class="data-table">
            ...
            <tbody>
                ...
                @empty
                    <tr class="empty-row"><td colspan="5">Aucun achat enregistré.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
