...
<div class="content-panel fiche-page">
    ...
    @include('partials.kpi-grid', ['cards' => [
        ['label' => $totalDebitLabel ?? 'Total Achats', 'value' => $totalAchats, 'unit' => 'MAD'],
    ...
    <form method="GET" class="filter-bar">
        ...
        <select name="{{ $tiersField }}" onchange="this.form.submit()">
            <option value="">— Tous les {{ strtolower($tiersLabel) }}s —</option>
            @foreach ($tiersList as $item)
                <option value="{{ $item->id }}" @selected(($selectedTiersId ?? null) === $item->id)>
                    {{ $item->nom_fournisseur ?? $item->nom_client }}
                </option>
            @endforeach
        </select>
    </form>
