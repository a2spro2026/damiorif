...
<style>
    ...
    .btn-gold {
        background: linear-gradient(135deg, #d4af37, #c9a45c 50%, #a8863f);
        color: var(--burgundy-deep);
        box-shadow: 0 4px 16px rgba(201, 164, 92, 0.3);
    }
    .btn-gold:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(201, 164, 92, 0.4); }
    .btn-ghost {
        background: rgba(0, 0, 0, 0.25); color: var(--gold-light);
        border-color: rgba(201, 164, 92, 0.35);
    }
    .btn-ghost:hover { background: rgba(201, 164, 92, 0.12); border-color: var(--gold); }
    .alert-error {
    ...
    .table-wrap {
        ...
        border: 1px solid rgba(201, 164, 92, 0.18); background: rgba(0, 0, 0, 0.22);
    }
    .data-table { width: 100%; border-collapse: collapse; min-width: 1100px; }
    .action-btns { display: flex; gap: 0.4rem; }
    .icon-btn {
        width: 34px; height: 34px; border-radius: 9px;
        border: 1px solid rgba(201, 164, 92, 0.3); background: rgba(0,0,0,0.3);
        color: var(--gold); display: inline-flex; align-items: center; justify-content: center;
        cursor: pointer; transition: all 0.2s ease;
    }
    .icon-btn:hover { background: rgba(201, 164, 92, 0.18); box-shadow: 0 0 12px rgba(201, 164, 92, 0.2); }
    .icon-btn.danger:hover { border-color: rgba(255,100,100,0.5); color: #ff9a9a; background: rgba(140,20,30,0.35); }
    .icon-btn svg { width: 16px; height: 16px; }
    .modal-backdrop {
        position: fixed; inset: 0; background: rgba(10,0,2,0.72); backdrop-filter: blur(6px);
    ...
    .modal-sheet {
        ...
        border: 1px solid rgba(201, 164, 92, 0.35); border-radius: 18px;
        box-shadow: 0 20px 60px rgba(0,0,0,0.55), 0 0 40px rgba(201,164,92,0.12);
        padding: 1.5rem;
    }
    .modal-header {
    ...
    .modal-header h3 { font-family: 'Playfair Display', serif; color: var(--gold); font-size: 1.25rem; }
    ...
    .field input, .field select {
        ...
        border: 1px solid rgba(201, 164, 92, 0.3); background: rgba(0,0,0,0.35);
        color: #fff; font-family: inherit; font-size: 0.92rem; outline: none;
    }
    .field input:focus, .field select:focus {
        border-color: var(--gold); box-shadow: 0 0 0 3px rgba(201,164,92,0.15);
    }
    ...
    .field .hint { font-size: 0.72rem; color: rgba(255,255,255,0.4); margin-top: 0.3rem; }
    .modal-footer {
        display: flex; justify-content: flex-end; gap: 0.65rem;
        margin-top: 1.25rem; padding-top: 1rem; border-top: 1px solid rgba(201,164,92,0.18);
    }
    .empty-row td { text-align: center; color: rgba(255,255,255,0.45); padding: 2rem; }
...
<div class="content-panel fiche-page">
    <div class="page-toolbar">
        <div>
            <h2>Fiche Client</h2>
            @if (!empty($depotLabel))
                <div class="page-meta">Dépôt : {{ $depotLabel }} — vos clients uniquement</div>
            @endif
        </div>
    ...
    <div class="table-wrap">
        <table class="data-table">
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-btns">
                                ...
                                <form method="POST" action="{{ route('clients.fiche.destroy', $f) }}" onsubmit="return confirm('Supprimer ce Client ?');" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr class="empty-row"><td colspan="10">Aucun Client enregistré.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-backdrop" id="cliModal">
        ...
        <form method="POST" id="cliForm">
            ...
            <div class="form-grid">
                ...
                <div class="field full">
                    <label for="field_nom">Nom Client</label>
                    <input type="text" name="nom_client" id="field_nom" required>
                </div>
                <div class="field">
                ...
                <div class="field full">
                    ...
                    <input type="text" name="rib" id="field_rib" maxlength="24" pattern="\d{24}" inputmode="numeric">
                    <div class="hint">24 chiffres (optionnel)</div>
                </div>
            </div>
...
<script>
    ...
    const storeUrl = @json(route('clients.fiche.store'));
    const updateBase = @json(url('/clients/fiche'));
    ...
    function fillForm(f) {
        ...
        document.getElementById('field_date').value = d
            ...
            : '—';
        document.getElementById('field_ref').value = f.ref_client || '';
        document.getElementById('field_nom').value = f.nom_client || '';
    ...
    function openViewModal(f) {
        document.getElementById('modalTitle').textContent = 'Détail Client';
        form.action = '#';
        fillForm(f);
