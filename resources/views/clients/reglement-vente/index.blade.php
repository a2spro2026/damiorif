...
<style>
    ...
    .btn-gold { background:linear-gradient(135deg,#d4af37,#c9a45c 50%,#a8863f); color:var(--burgundy-deep); box-shadow:0 4px 16px rgba(201,164,92,.3); }
    ...
    .icon-btn { width:32px; height:32px; border-radius:8px; border:1px solid rgba(201,164,92,.3); background:rgba(0,0,0,.3); color:var(--gold); display:inline-flex; align-items:center; justify-content:center; cursor:pointer; text-decoration:none; }
    .icon-btn:hover { background:rgba(201,164,92,.18); }
    ...
    .modal-sheet { width:min(1100px,100%); margin:1rem auto 2rem; background:linear-gradient(160deg,rgba(84,0,11,.98),rgba(45,0,6,.99)); border:1px solid rgba(201,164,92,.35); border-radius:18px; box-shadow:0 20px 60px rgba(0,0,0,.55); padding:1.35rem 1.5rem; }
    ...
    .field input:focus,.field select:focus { border-color:var(--gold); box-shadow:0 0 0 3px rgba(201,164,92,.12); }
    ...
    .bons-table input { width:100%; padding:.45rem .5rem; border-radius:8px; border:1px solid rgba(201,164,92,.25); background:rgba(0,0,0,.3); color:#fff; font-size:.82rem; font-family:inherit; text-align:center; }
    .selected-table { width:100%; border-collapse:collapse; min-width:700px; margin-top:.5rem; }
    .totals-bar { display:flex; justify-content:flex-end; gap:1.5rem; margin-top:.85rem; padding-top:.75rem; border-top:1px solid rgba(201,164,92,.18); color:var(--gold-light); font-weight:700; }
    ...
    .bons-hint { color:rgba(255,255,255,.45); font-size:.82rem; margin:.35rem 0 .75rem; }
            ...
            <tbody>
                @forelse ($reglements as $r)
                    <tr>
                        ...
                        <td>{{ $r->date_encaissement?->format('d/m/Y') ?: '—' }}</td>
                        <td>
                            @php $statutKey = $r->statut ?: 'en_instance'; @endphp
                            <form method="POST" action="{{ route('clients.reglement_vente.statut', $r) }}" class="statut-form">
                                @csrf
                                @method('PATCH')
                        ...
                        <td>
                            <div class="action-btns">
                                ...
                                <form method="POST" action="{{ route('clients.reglement_vente.destroy', $r) }}" onsubmit="return confirm('Supprimer ce réglement ?');" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a href="{{ route('clients.reglement_vente.print', $r) }}" target="_blank" class="icon-btn" title="Imprimer">
                ...
                @empty
                    <tr class="empty-row"><td colspan="10">Aucun réglement vente enregistré.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-backdrop" id="raModal">
    <div class="modal-sheet">
        <div class="modal-header">
            <h3 id="modalTitle">Ajouter un réglement vente</h3>
        ...
        <form method="POST" id="raForm">
            ...
            <div class="form-grid">
                <div class="form-row form-row-top">
                    ...
                    <div class="field">
                        <label for="field_client">Nom Client</label>
                        <select name="client_id" id="field_client" required onchange="onClientChange()">
                            ...
                            @foreach ($clients as $f)
                                <option
                                    ...
                                    data-nom="{{ $f->nom_client }}"
                                >{{ $f->nom_client }} ({{ $f->ref_client }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field">
                ...
                <div class="form-row form-row-pay">
                    ...
                    <div class="field">
                        <label for="field_decaiss">Date Encaiss</label>
                        <input type="date" name="date_encaissement" id="field_decaiss">
                    </div>
                    <div class="field">
            ...
            <p class="bons-hint" id="bonsHint">Sélectionnez un client pour afficher ses bons non soldés.</p>
...
<script>
    ...
    const storeUrl = @json(route('clients.reglement_vente.store'));
    const updateBase = @json(url('/clients/reglement-vente'));
    ...
    function onClientChange() {
        const opt = document.getElementById('field_client').selectedOptions[0];
    ...
    function renderUnpaidBons() {
        const frnsId = document.getElementById('field_client').value;
        ...
        if (!frnsId) {
            ...
            hint.textContent = 'Sélectionnez un client pour afficher ses bons non soldés.';
            return;
        }

        const bons = bonsNonSoldes
            .filter(b => String(b.client_id) === String(frnsId))
            .map(b => ({ ...b, solde_dispo: availableSolde(b) }))
            .filter(b => b.solde_dispo > 0.009 || selectedLignes.some(l => String(l.bon_vente_id) === String(b.id)));

        // Include bons already selected that may be missing from unpaid list
        selectedLignes.forEach(l => {
            if (!bons.some(b => String(b.id) === String(l.bon_vente_id))) {
                const found = bonsNonSoldes.find(b => String(b.id) === String(l.bon_vente_id));
                if (found) {
                    ...
                    bons.push({
                        ...
                        solde: 0,
                        solde_dispo: (editBonusByBonId[l.bon_vente_id] || 0),
                        client_id: frnsId,
                    });
                }
            }
        ...
        if (!bons.length) {
            wrap.style.display = 'none';
            hint.style.display = 'block';
            hint.textContent = 'Aucun bon non soldé pour ce client.';
            return;
        }
        ...
        bons.forEach(bon => {
            const already = selectedLignes.find(l => String(l.bon_vente_id) === String(bon.id));
    ...
    function collectLignesFromBons() {
        ...
        document.querySelectorAll('#bonsBody tr').forEach(tr => {
            ...
            lignes.push({
                bon_vente_id: amountInput.dataset.id,
    ...
    function syncHiddenLignes() {
        ...
        lignes.forEach((l, i) => {
            ...
            h1.type = 'hidden';
            h1.name = `lignes[${i}][bon_vente_id]`;
            h1.value = l.bon_vente_id;
    ...
    function addSelectedBons() {
        ...
        if (!list.length) {
            alert('Sélectionnez un client avec des bons non soldés.');
            return;
        }
    ...
    function resetForm() {
        ...
        document.getElementById('field_client').value = '';
    ...
    function openCreateModal() {
        document.getElementById('modalTitle').textContent = "Ajouter un réglement vente";
    ...
    function fillForm(r) {
        ...
        document.getElementById('field_client').value = r.client_id || '';
        ...
        document.getElementById('field_decaiss').value = (r.date_encaissement || '').substring(0, 10);
        ...
        selectedLignes = (r.lignes || []).map(l => {
            editBonusByBonId[l.bon_vente_id] = parseFloat(l.montant_applique) || 0;
            return {
                bon_vente_id: l.bon_vente_id,
                numero_bon: l.numero_bon,
    ...
    function openEditModal(r) {
        document.getElementById('modalTitle').textContent = 'Modifier réglement vente';
    ...
    function openViewModal(r) {
        document.getElementById('modalTitle').textContent = 'Détail réglement vente';
        form.action = '#';
