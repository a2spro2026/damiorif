...
<style>
    ...
    .lines-table input { width:100%; padding:.45rem .5rem; border-radius:8px; border:1px solid rgba(201,164,92,.25); background:rgba(0,0,0,.3); color:#fff; font-size:.82rem; font-family:inherit; }
...
<div class="modal-backdrop" id="baModal">
    <div class="modal-sheet">
        <div class="modal-header">
            <h3 id="modalTitle">Ajouter un bon de vente</h3>
        ...
        <form method="POST" id="baForm">
            ...
            <div class="form-grid">
                ...
                <div class="form-row form-row-client">
                    ...
                    <div class="field">
                        <label for="field_client_id">ID Client</label>
                        <select name="client_id" id="field_client_id" required onchange="onClientChange()">
                            ...
                            @foreach ($clients as $f)
                                ...
                                >{{ $f->id }} — {{ $f->ref_client }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="field field-nom">
                        <label for="field_client_nom">Nom Client</label>
                        <input type="text" id="field_client_nom" readonly>
                    </div>
                </div>
                <div class="form-row form-row-meta">
                    ...
                    <div class="field">
                        ...
                        <select name="depot" id="field_depot" @if(!empty($lockedDepot)) disabled @endif>
                            <option value="">— Sélectionner —</option>
                            @foreach ($depots as $value => $label)
                                <option value="{{ $value }}" @selected(($lockedDepot ?? null) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @if (!empty($lockedDepot))
...
<script>
    ...
    function onClientChange() {
        const opt = document.getElementById('field_client_id').selectedOptions[0];
        document.getElementById('field_client_nom').value = opt?.dataset?.nom || '';
    ...
    function setFormReadonly(readonly) {
        ...
        form.querySelectorAll('input, select, button').forEach(el => {
            if (el.id === 'field_numero' || el.id === 'field_client_nom' || el.id === 'field_ville') return;
            ...
            if (el.id === 'field_depot' && lockedDepot) {
                el.disabled = true;
                return;
            }
    ...
    function openCreateModal() {
        ...
        document.getElementById('field_client_nom').value = '';
        ...
        document.getElementById('field_depot').value = lockedDepot || '';
        clearLines();
        setFormReadonly(false);
        addLine();
        openModal();
    ...
    function fillForm(bon) {
        ...
        document.getElementById('field_client_id').value = bon.client_id || '';
        onClientChange();
        document.getElementById('field_type').value = bon.type_reglement || '';
        document.getElementById('field_echeance').value = bon.echeance || '';
        document.getElementById('field_depot').value = bon.depot || lockedDepot || '';
        clearLines();
